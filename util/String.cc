...
String String::getNextNchars(unsigned nChar)
{
  ...
  if(str_.size() - iStart_ < nChar)
    ThrowError("Remaining string is too short (" << (str_.size()-iStart_) << ") to retrieve " << nChar << " chars.");
...
bool String::toBool()
{
  ...
  if(str == "true") {
    ...
    ThrowError("Unrecognized boolean value: " << str_);
    return false;
  }
}
...
double String::getPowerExpression()
{
  if(contains('^')) {
    ...
    if(base.isEmpty())
      ThrowError("String does not contain a valid power expression");
...
void String::advance(unsigned nChar)
{
  ...
  ThrowError("Advancing by " << nChar << " would exceed the string length");
}
