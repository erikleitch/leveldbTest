...
bool DBImpl::GetProperty(const Slice& property, std::string* value) {
  ...
  std::cout << "Inside GetPr" << std::endl;
  value->clear();
  ...
  if (in.starts_with("num-files-at-level")) {
    ...
    Logger::Stats stats;
    options_.info_log->getStats(stats);

    double nMb  = stats.nBytesWritten_ / 1048576.0;
    double nSec = stats.nMicroSeconds_ / 1e6;

    char buf[200];
    snprintf(buf, sizeof(buf),
             "                               Log Writes\n"
             "Writes(#) Time(%s) Write(%s)\n"
             "--------------------------------------------------\n",
	     (nSec >= 1 ? "sec" : "msec"),
	     (nMb  >= 1 ? "MB"  : "B"));
    value->append(buf);

    if(stats.nMicroSeconds_ > 0) {
      snprintf(
	       buf, sizeof(buf),
	       "%3llu %8.0f %8.0f\n",
	       stats.nWrites_,
	       (nMb  >= 1 ? nMb  : (double)stats.nBytesWritten_),
	       (nSec >= 1 ? nSec : (double)stats.nMicroSeconds_));
      value->append(buf);
    }
